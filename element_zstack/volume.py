...
class Segmentation(dj.Computed):
    ...
    class Flows(dj.Part):
        """Flows generated by segmentation.

        Attributes:
            Segmentation (foreign key): Primary key from `Segmentation`.
            xy_flow (longblob): XY flow in RGB.
            cell_probability (longblob): Cell probability in range 0-255.
            z_flow (longblob): Z flow in range 0-255.
            flow_components (longblob): (dZ), dY, dX, cellprob.
        """
        definition = """ # Flow fields generated by segmentation
        -> master
        ---
        xy_flow             : longblob  # XY flow in RGB
        cell_probability    : longblob  # cell probability in range 0-255
    ...
    def make(self, key):
        ...
        try:
            output_dir = find_full_path(
                get_volume_root_data_dir(), output_dir
            ).as_posix()
